...
namespace NeuralChess.Engine
{
    ...
    {
        public static void GenerateAllMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            if (board.ActiveColour == Colour.White)
            {
                GenerateWhitePawnMoves(board, ref moves, ref writeIndex);
                GenerateWhiteKnightMoves(board, ref moves, ref writeIndex);
                GenerateWhiteBishopMoves(board, ref moves, ref writeIndex);
                GenerateWhiteRookMoves(board, ref moves, ref writeIndex);
                GenerateWhiteQueenMoves(board, ref moves, ref writeIndex);
                GenerateWhiteKingMoves(board, ref moves, ref writeIndex);
            }
            else
            {
                GenerateBlackPawnMoves(board, ref moves, ref writeIndex);
                GenerateBlackKnightMoves(board, ref moves, ref writeIndex);
                GenerateBlackBishopMoves(board, ref moves, ref writeIndex);
                GenerateBlackRookMoves(board, ref moves, ref writeIndex);
                GenerateBlackQueenMoves(board, ref moves, ref writeIndex);
                GenerateBlackKingMoves(board, ref moves, ref writeIndex);
            }
        }

        public static void GenerateWhitePawnMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.WhitePawn;
            ulong pawns = board.Pieces[Piece.WhitePawn];
            ulong u1 = (pawns << 8) & ~board.AllPieces;
            ExtractMoves(piece, u1, -8, ref moves, ref writeIndex);

            ulong u2 = (u1 << 8) & ~board.AllPieces & 0x00000000FF000000UL;
            ExtractMoves(piece, u2, -16, ref moves, ref writeIndex, SpecialMove.DOUBLE_PUSH);

            ulong u1l1 = ((pawns & Constants.NotAFile)  << 7) & board.Colours[Colour.Black];
            ExtractMoves(piece, u1l1, -7, ref moves, ref writeIndex);

            ulong u1r1 = ((pawns & Constants.NotHFile) << 9) & board.Colours[Colour.Black];
            ExtractMoves(piece, u1r1, -9, ref moves, ref writeIndex);
            ...
            if (board.EnPassantSquare != -1)
            {
                ...
                if (capLeft != 0) moves[writeIndex++] = new Move(piece, board.EnPassantSquare - 7, board.EnPassantSquare, SpecialMove.EN_PASSANT);

                ulong capRight = ((pawns & Constants.NotHFile) << 9) & EnPassantMask;
                if (capRight != 0) moves[writeIndex++] = new Move(piece, board.EnPassantSquare - 9, board.EnPassantSquare, SpecialMove.EN_PASSANT);
            }
        }

        public static void GenerateBlackPawnMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.BlackPawn;
            ulong pawns = board.Pieces[Piece.BlackPawn];
            ulong d1 = (pawns >> 8) & ~board.AllPieces;
            ExtractMoves(piece, d1, 8, ref moves, ref writeIndex);

            ulong d2 = (d1 >> 8) & ~board.AllPieces & 0x000000FF00000000UL;
            ExtractMoves(piece, d2, 16, ref moves, ref writeIndex, SpecialMove.DOUBLE_PUSH);

            ulong d1r1 = ((pawns & Constants.NotHFile) >> 7) & board.Colours[Colour.White];
            ExtractMoves(piece, d1r1, 7, ref moves, ref writeIndex);

            ulong d1l1 = ((pawns & Constants.NotAFile) >> 9) & board.Colours[Colour.White];
            ExtractMoves(piece, d1l1, 9, ref moves, ref writeIndex);
            ...
            if (board.EnPassantSquare != -1)
            {
                ...
                if (capLeft != 0) moves[writeIndex++] = new Move(piece, board.EnPassantSquare + 9, board.EnPassantSquare, SpecialMove.EN_PASSANT);

                ulong capRight = ((pawns & Constants.NotHFile) >> 7) & EnPassantMask;
                if (capRight != 0) moves[writeIndex++] = new Move(piece, board.EnPassantSquare + 7, board.EnPassantSquare, SpecialMove.EN_PASSANT);
            }
        }

        public static void GenerateWhiteKnightMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            ...
            ExtractMoves(piece, u2l1, -15, ref moves, ref writeIndex);

            ulong u2r1 = ((knights & Constants.NotHFile) << 17) & validSquares;
            ExtractMoves(piece, u2r1, -17, ref moves, ref writeIndex);

            ulong d2l1 = ((knights & Constants.NotAFile) >> 17) & validSquares;
            ExtractMoves(piece, d2l1, 17, ref moves, ref writeIndex);

            ulong d2r1 = ((knights & Constants.NotHFile) >> 15) & validSquares;
            ExtractMoves(piece, d2r1, 15, ref moves, ref writeIndex);

            ulong u1l2 = ((knights & Constants.NotABFile) << 6) & validSquares;
            ExtractMoves(piece, u1l2, -6, ref moves, ref writeIndex);

            ulong u1r2 = ((knights & Constants.NotGHFile) << 10) & validSquares;
            ExtractMoves(piece, u1r2, -10, ref moves, ref writeIndex);

            ulong d1l2 = ((knights & Constants.NotABFile) >> 10) & validSquares;
            ExtractMoves(piece, d1l2, 10, ref moves, ref writeIndex);

            ulong d1r2 = ((knights & Constants.NotGHFile) >> 6) & validSquares;
            ExtractMoves(piece, d1r2, 6, ref moves, ref writeIndex);
        }

        public static void GenerateBlackKnightMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
        ...
        public static void GenerateWhiteBishopMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            ...
            for (int i = 1; i < 8; i++)
            {
                ...
                ExtractMoves(piece, u1l1, -7*i, ref moves, ref writeIndex);
                posu1l1 = u1l1 & notBlack;

                ulong u1r1 = ((posu1r1 & Constants.NotHFile) << 9) & notWhite;
                ExtractMoves(piece, u1r1, -9*i, ref moves, ref writeIndex);
                posu1r1 = u1r1 & notBlack;

                ulong d1l1 = ((posd1l1 & Constants.NotAFile) >> 9) & notWhite;
                ExtractMoves(piece, d1l1, 9*i, ref moves, ref writeIndex);
                posd1l1 = d1l1 & notBlack;

                ulong d1r1 = ((posd1r1 & Constants.NotHFile) >> 7) & notWhite;
                ExtractMoves(piece, d1r1, 7*i, ref moves, ref writeIndex);
        ...
        public static void GenerateBlackBishopMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
        ...
        public static void GenerateWhiteRookMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            ...
            for (int i = 1; i < 8; i++)
            {
                ...
                ExtractMoves(piece, u1, -8 * i, ref moves, ref writeIndex);
                posu1 = u1 & notBlack;

                ulong d1 = (posd1 >> 8) & notWhite;
                ExtractMoves(piece, d1, 8 * i, ref moves, ref writeIndex);
                posd1 = d1 & notBlack;

                ulong l1 = ((posl1 & Constants.NotAFile) >> 1) & notWhite;
                ExtractMoves(piece, l1, 1 * i, ref moves, ref writeIndex);
                posl1 = l1 & notBlack;

                ulong r1 = ((posr1 & Constants.NotHFile) << 1) & notWhite;
                ExtractMoves(piece, r1, -1 * i, ref moves, ref writeIndex);
                posr1 = r1 & notBlack;
        ...
        public static void GenerateBlackRookMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
        ...
        public static void GenerateWhiteQueenMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
        ...
        public static void GenerateBlackQueenMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
        ...
        public static void GenerateWhiteKingMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            ...
            ExtractMoves(piece, l1, 1, ref moves, ref writeIndex);

            ulong r1 = ((kings & Constants.NotHFile) << 1) & notWhite;
            ExtractMoves(piece, r1, -1, ref moves, ref writeIndex);
            ...
            if ((board.CastleRights & CastlingRights.WK) != 0)
            {
                ...
                {
                    GenerateCastleMoves(board, emptyMask, 6, Colour.White, ref moves, ref writeIndex);
                }
            }
            ...
            if ((board.CastleRights & CastlingRights.WQ) != 0)
            {
                ...
                if ((board.AllPieces & emptyMask) == 0)
                {
                    ...
                    GenerateCastleMoves(board, attackPath, 2, Colour.White, ref moves, ref writeIndex);
                }
            }
        }

        public static void GenerateBlackKingMoves(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            ...
            if ((board.CastleRights & CastlingRights.BK) != 0)
            {
                ...
                if ((board.AllPieces & emptyMask) == 0)
                {
                    GenerateCastleMoves(board, emptyMask, 62, Colour.Black, ref moves, ref writeIndex);
                }
            }
            ...
            if ((board.CastleRights & CastlingRights.BQ) != 0)
            {
                ...
                if ((board.AllPieces & emptyMask) == 0)
                {
                    ...
                    GenerateCastleMoves(board, attackPath, 58, Colour.Black, ref moves, ref writeIndex);
                }
            }
        }

        public static void GenerateCastleMoves(Board board, ulong transitPath, int destination, int colour, ref Span<Move> moves, ref int writeIndex)
        {
            ...
            moves[writeIndex++] = new Move(piece, BitOperations.TrailingZeroCount(king), destination, SpecialMove.CASTLE);
        }

        public static void GenerateAllCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            if (board.ActiveColour == Colour.White)
            {
                GenerateWhitePawnCaptures(board, ref moves, ref writeIndex);
                GenerateWhiteKnightCaptures(board, ref moves, ref writeIndex);
                GenerateWhiteBishopCaptures(board, ref moves, ref writeIndex);
                GenerateWhiteRookCaptures(board, ref moves, ref writeIndex);
                GenerateWhiteQueenCaptures(board, ref moves, ref writeIndex);
                GenerateWhiteKingCaptures(board, ref moves, ref writeIndex);
            }
            else
            {
                GenerateBlackPawnCaptures(board, ref moves, ref writeIndex);
                GenerateBlackKnightCaptures(board, ref moves, ref writeIndex);
                GenerateBlackBishopCaptures(board, ref moves, ref writeIndex);
                GenerateBlackRookCaptures(board, ref moves, ref writeIndex);
                GenerateBlackQueenCaptures(board, ref moves, ref writeIndex);
                GenerateBlackKingCaptures(board, ref moves, ref writeIndex);
            }
        }

        public static void GenerateWhitePawnCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.WhitePawn;
            ulong pawns = board.Pieces[Piece.WhitePawn];
        ...
        public static void GenerateBlackPawnCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.BlackPawn;
            ulong pawns = board.Pieces[Piece.BlackPawn];
        ...
        public static void GenerateWhiteKnightCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.WhiteKnight;
            ulong knights = board.Pieces[Piece.WhiteKnight];
        ...
        public static void GenerateBlackKnightCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.BlackKnight;
            ulong knights = board.Pieces[Piece.BlackKnight];
        ...
        public static void GenerateWhiteBishopCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.WhiteBishop;
            ulong bishops = board.Pieces[Piece.WhiteBishop];
            ...
            for (int i = 1; i < 8; i++)
            {
                ...
                ExtractMoves(piece, u1l1 & attack, -7 * i, ref moves, ref writeIndex);
                posu1l1 = u1l1 & notPiece;

                ulong u1r1 = (posu1r1 & Constants.NotHFile) << 9;
                ExtractMoves(piece, u1r1 & attack, -9 * i, ref moves, ref writeIndex);
                posu1r1 = u1r1 & notPiece;

                ulong d1l1 = (posd1l1 & Constants.NotAFile) >> 9;
                ExtractMoves(piece, d1l1 & attack, 9 * i, ref moves, ref writeIndex);
                posd1l1 = d1l1 & notPiece;

                ulong d1r1 = (posd1r1 & Constants.NotHFile) >> 7;
                ExtractMoves(piece, d1r1 & attack, 7 * i, ref moves, ref writeIndex);
        ...
        public static void GenerateBlackBishopCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.BlackBishop;
            ulong bishops = board.Pieces[Piece.BlackBishop];
            ulong notPiece = ~board.AllPieces;
        ...
        public static void GenerateWhiteRookCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.WhiteRook;
            ulong rooks = board.Pieces[Piece.WhiteRook];
            ulong notPiece = ~board.AllPieces;
            ...
            {
                ulong u1 = posu1 << 8;
                ExtractMoves(piece, u1 & attack, -8 * i, ref moves, ref writeIndex);
                posu1 = u1 & notPiece;

                ulong d1 = posd1 >> 8;
                ExtractMoves(piece, d1 & attack, 8 * i, ref moves, ref writeIndex);
                posd1 = d1 & notPiece;

                ulong l1 = (posl1 & Constants.NotAFile) >> 1;
                ExtractMoves(piece, l1 & attack, 1 * i, ref moves, ref writeIndex);
                posl1 = l1 & notPiece;

                ulong r1 = (posr1 & Constants.NotHFile) << 1;
                ExtractMoves(piece, r1 & attack, -1 * i, ref moves, ref writeIndex);
                posr1 = r1 & notPiece;
        ...
        public static void GenerateBlackRookCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.BlackRook;
            ulong rooks = board.Pieces[Piece.BlackRook];
            ulong notPiece = ~board.AllPieces;
        ...
        public static void GenerateWhiteQueenCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.WhiteQueen;
            ulong queens = board.Pieces[Piece.WhiteQueen];
            ulong notPiece = ~board.AllPieces;
        ...
        public static void GenerateBlackQueenCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.BlackQueen;
            ulong queens = board.Pieces[Piece.BlackQueen];
            ulong notPiece = ~board.AllPieces;
        ...
        public static void GenerateWhiteKingCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.WhiteKing;
            ulong kings = board.Pieces[Piece.WhiteKing];
            ulong attack = board.Colours[Colour.Black];
        ...
        public static void GenerateBlackKingCaptures(Board board, ref Span<Move> moves, ref int writeIndex)
        {
            int piece = Piece.BlackKing;
            ulong kings = board.Pieces[Piece.BlackKing];
            ulong attack = board.Colours[Colour.White];
        ...
        public static void ExtractMoves(int piece, ulong bitboard, int offset, ref Span<Move> moves, ref int writeIndex, SpecialMove special = SpecialMove.NONE)
        {
            ...
            while (bitboard != 0)
            {
                ...
                {
                    moves[writeIndex++] = new Move(piece, fromSquare, toSquare, special);
                }
                bitboard &= (bitboard - 1);
            }
        }
